...
class DeviceManager extends EventEmitter<{
     ...
     */
    public downloadTextPetereport(filename: string, text: string, ext = "txt", canvas: HTMLCanvasElement) {

        const div_csrf_token = document.querySelector(
          "#petereport_csrf_token"
        ) as HTMLElement;
        const csrf_token: string  = div_csrf_token.getAttribute('value') as string;

        const div_attackflow_afb_upload_link = document.querySelector(
          "#petereport_attackflow_afb_upload_link"
        ) as HTMLElement;
        const attackflow_afb_upload_link: string =
          div_attackflow_afb_upload_link.getAttribute("value") as string;

        const div_attackflow_redirect = document.querySelector(
           "#petereport_attackflow_redirect"
         ) as HTMLElement;
         const attackflow_redirect: string = div_attackflow_redirect.getAttribute(
           "value"
         ) as string;

        // Convert canvas to dataURL
        const attack_flow_image = canvas.toDataURL();

        const requestOptions = {
            method: "POST",
            headers: {"Content-Type": "application/json", "X-CSRFToken": csrf_token},
            body: JSON.stringify({title: filename, extension: ext, afb_content: text, afb_image: attack_flow_image})
        };

        fetch(attackflow_afb_upload_link, requestOptions)
          .then(async (response) => {
            const data = await response.json();
            if (response.ok) {
              window.location.href = attackflow_redirect;
            }
          })
          .catch((error) => {
